...
int main() {
    ...
    unsigned char *data = stbi_load("./textures/blackhole.png", &width, &height, &nrChannels, 0);
    if (data) {
    ...
    data = stbi_load("./textures/galaxy.png", &width, &height, &nrChannels, 0);
    if (data) {
        glTexImage2D(GL_TEXTURE_2D, 0,GL_RGB, width, height, 0,GL_RGB, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
    } else {
    ...
    rectangleShader.use();
    rectangleShader.setInt("texture0", 0);
    rectangleShader.setInt("texture1", 1);
